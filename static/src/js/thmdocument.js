odoo.define('thmdocument.update_kanban', function (require) {
'use strict';
...
KanbanRecord.include({
    on_card_clicked: function () {
        if (this.model === 'thmdocument.thmdocument') {
            this.$('.o_thmdocument_kanban_boxes a').first().click();
        } else {
    ...
    on_kanban_action_clicked: function (ev) {
        var self = this;
        if (this.model === 'thmdocument.task' && $(ev.currentTarget).data('type') === 'set_cover') {
            ev.preventDefault();

            new Model('ir.attachment').query(['id', 'name'])
               .filter([['res_model', '=', 'thmdocument.task'], ['res_id', '=', this.id], ['mimetype', 'ilike', 'image']])
        ...
        function open_cover_images_dialog(attachment_ids) {
            ...
            var $content = $(QWeb.render("thmdocument.SetCoverModal", {
                cover_id: cover_id,
