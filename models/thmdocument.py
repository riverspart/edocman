...
class ThmdocumentTaskType(models.Model):
    ...
    def _get_mail_template_id_domain(self):
        return [('model', '=', 'thmdocument.task')]

    def _get_default_thmdocument_ids(self):
        default_thmdocument_id = self.env.context.get('default_thmdocument_id')
        return [default_thmdocument_id] if default_thmdocument_id else None

    name = fields.Char(string='Stage Name', required=True, translate=True)
    description = fields.Text(translate=True)
    sequence = fields.Integer(default=1)
    thmdocument_ids = fields.Many2many('thmdocument.thmdocument', 'thmdocument_task_type_rel', 'type_id', 'thmdocument_id', string='Thmdocuments',
        default=_get_default_thmdocument_ids)
...
class Thmdocument(models.Model):
    ...
    def get_alias_values(self):
        values = super(Thmdocument, self).get_alias_values()
        values['alias_defaults'] = {'thmdocument_id': self.id}
        return values
    ...
    def _compute_attached_docs_count(self):
        ...
        for thmdocument in self:
            thmdocument.doc_count = Attachment.search_count([
                '|',
                '&',
                ('res_model', '=', 'thmdocument.thmdocument'), ('res_id', '=', thmdocument.id),
                '&',
                ('res_model', '=', 'thmdocument.task'), ('res_id', 'in', thmdocument.task_ids.ids)
            ])

    def _compute_task_count(self):
        for thmdocument in self:
            thmdocument.task_count = len(thmdocument.task_ids)

    def _compute_task_needaction_count(self):
        thmdocuments_data = self.env['thmdocument.task'].read_group([
            ('thmdocument_id', 'in', self.ids),
            ('message_needaction', '=', True)
        ], ['thmdocument_id'], ['thmdocument_id'])
        mapped_data = {thmdocument_data['thmdocument_id'][0]: int(thmdocument_data['thmdocument_id_count'])
                       for thmdocument_data in thmdocuments_data}
        for thmdocument in self:
            thmdocument.task_needaction_count = mapped_data.get(thmdocument.id, 0)
    ...
    def attachment_tree_view(self):
        ...
        domain = [
            '|',
            '&', ('res_model', '=', 'thmdocument.thmdocument'), ('res_id', 'in', self.ids),
            '&', ('res_model', '=', 'thmdocument.task'), ('res_id', 'in', self.task_ids.ids)]
        return {
            'name': _('Attachments'),
            'domain': domain,
    ...
    @api.model
    def activate_sample_thmdocument(self):
        """ Unarchives the sample thmdocument 'thmdocument.thmdocument_thmdocument_data' and
            reloads the thmdocument dashboard """
        # Unarchive sample thmdocument
        thmdocument = self.env.ref('thmdocument.thmdocument_thmdocument_data', False)
        if thmdocument:
            thmdocument.write({'active': True})

        cover_image = self.env.ref('thmdocument.msg_task_data_14_attach', False)
        cover_task = self.env.ref('thmdocument.thmdocument_task_data_14', False)
        if cover_image and cover_task:
            cover_task.write({'displayed_image_id': cover_image.id})

        # Change the help message on the action (no more activate thmdocument)
        action = self.env.ref('thmdocument.open_view_thmdocument_all', False)
        action_data = None
        if action:
            action.sudo().write({
    ...
    def _compute_is_favorite(self):
        for thmdocument in self:
            thmdocument.is_favorite = self.env.user in thmdocument.favorite_user_ids
    ...
    analytic_account_id = fields.Many2one(
        ...
        help="Link this thmdocument to an analytic account if you need financial management on thmdocuments. "
             "It enables you to connect thmdocuments with budgets, planning, cost and revenue analysis, timesheets on thmdocuments, etc.",
        ondelete="cascade", required=True, auto_join=True)
    favorite_user_ids = fields.Many2many(
        'res.users', 'thmdocument_favorite_user_rel', 'thmdocument_id', 'user_id',
    ...
    type_ids = fields.Many2many('thmdocument.task.type', 'thmdocument_task_type_rel', 'thmdocument_id', 'type_id', string='Tasks Stages')
    task_count = fields.Integer(compute='_compute_task_count', string="Tasks")
    task_needaction_count = fields.Integer(compute='_compute_task_needaction_count', string="Tasks")
    task_ids = fields.One2many('thmdocument.task', 'thmdocument_id', string='Tasks',
                               domain=['|', ('stage_id.fold', '=', False), ('stage_id', '=', False)])
    color = fields.Integer(string='Color Index')
    user_id = fields.Many2one('res.users', string='Thmdocument Manager', default=lambda self: self.env.user)
    alias_id = fields.Many2one('mail.alias', string='Alias', ondelete="restrict", required=True,
        help="Internal email associated with this thmdocument. Incoming emails are automatically synchronized "
             "with Tasks (or optionally Issues if the Issue Tracker module is installed).")
    alias_model = fields.Selection(_alias_models, string="Alias Model", index=True, required=True, default='thmdocument.task',
        help="The kind of document created when an email is received on this thmdocument's email alias")
    privacy_visibility = fields.Selection([
        ...
        default='employees',
        help="Holds visibility of the tasks or issues that belong to the current thmdocument:\n"
                "- On invitation only: Employees may only see the followed thmdocument, tasks or issues\n"
                "- Visible by all employees: Employees may see all thmdocument, tasks or issues\n"
                "- Visible by following customers: employees see everything;\n"
                "   if website is activated, portal users may see thmdocument, tasks or issues followed by\n"
    ...
    _sql_constraints = [
        ('thmdocument_date_greater', 'check(date >= date_start)', 'Error! thmdocument start-date must be lower than thmdocument end-date.')
    ]

    @api.multi
    def map_tasks(self, new_thmdocument_id):
        """ copy and map tasks from old to new thmdocument """
        tasks = self.env['thmdocument.task']
        for task in self.tasks:
            # preserve task name and stage, normally altered during copy
            defaults = {'stage_id': task.stage_id.id,
                        'name': task.name}
            tasks += task.copy(defaults)
        return self.browse(new_thmdocument_id).write({'tasks': [(6, 0, tasks.ids)]})
    ...
    def copy(self, default=None):
        ...
        thmdocument = super(Thmdocument, self).copy(default)
        for follower in self.message_follower_ids:
            thmdocument.message_subscribe(partner_ids=follower.partner_id.ids, subtype_ids=follower.subtype_ids.ids)
        self.map_tasks(thmdocument.id)
        return thmdocument

    @api.model
    def create(self, vals):

        ir_values = self.env['ir.values'].get_default('thmdocument.config.settings', 'generate_thmdocument_alias')
        if ir_values:
            vals['alias_name'] = vals.get('alias_name') or vals.get('name')
        # Prevent double thmdocument creation when 'use_tasks' is checked
        self = self.with_context(thmdocument_creation_in_progress=True, mail_create_nosubscribe=True)
        return super(Thmdocument, self).create(vals)
    ...
    def write(self, vals):
        ...
        if vals.get('alias_model'):
            vals['alias_model_id'] = self.env['ir.model'].search([
                ('model', '=', vals.get('alias_model', 'thmdocument.task'))
            ], limit=1).id
        res = super(Thmdocument, self).write(vals)
        if 'active' in vals:
            # archiving/unarchiving a thmdocument does it on its tasks, too
            self.with_context(active_test=False).mapped('tasks').write({'active': vals['active']})
        return res

    @api.multi
    def toggle_favorite(self):
        favorite_thmdocuments = not_fav_thmdocuments = self.env['thmdocument.thmdocument'].sudo()
        for thmdocument in self:
            if self.env.user in thmdocument.favorite_user_ids:
                favorite_thmdocuments |= thmdocument
            else:
                not_fav_thmdocuments |= thmdocument

        # Thmdocument User has no write access for thmdocument.
        not_fav_thmdocuments.write({'favorite_user_ids': [(4, self.env.uid)]})
        favorite_thmdocuments.write({'favorite_user_ids': [(3, self.env.uid)]})
...
class Task(models.Model):
    ...
    def _get_default_partner(self):
        if 'default_thmdocument_id' in self.env.context:
            ...
            return default_thmdocument_id.exists().partner_id

    def _get_default_stage_id(self):
        """ Gives default stage_id """
        thmdocument_id = self.env.context.get('default_thmdocument_id')
        if not thmdocument_id:
            return False
        return self.stage_find(thmdocument_id, [('fold', '=', False)])

    @api.model
    def _read_group_stage_ids(self, stages, domain, order):
        search_domain = [('id', 'in', stages.ids)]
        if 'default_thmdocument_id' in self.env.context:
            search_domain = ['|', ('thmdocument_ids', '=', self.env.context['default_thmdocument_id'])] + search_domain
    ...
    stage_id = fields.Many2one('thmdocument.task.type', string='Stage', track_visibility='onchange', index=True,
        default=_get_default_stage_id, group_expand='_read_group_stage_ids',
        domain="[('thmdocument_ids', '=', thmdocument_id)]", copy=False)
    tag_ids = fields.Many2many('thmdocument.tags', string='Tags', oldname='categ_ids')
    ...
    date_last_stage_update = fields.Datetime(string='Last Stage Update',
        ...
        copy=False,
        readonly=True)
    thmdocument_id = fields.Many2one('thmdocument.thmdocument',
        string='Thmdocument',
        default=lambda self: self.env.context.get('default_thmdocument_id'),
        index=True,
        track_visibility='onchange',
        change_default=True)
    notes = fields.Text(string='Notes')
    planned_hours = fields.Float(string='Initially Planned Hours', help='Estimated time to do the task, usually set by the thmdocument manager when the task is in draft state.')
    ...
    manager_id = fields.Many2one('res.users', string='Thmdocument Manager', related='thmdocument_id.user_id', readonly=True)
    ...
    displayed_image_id = fields.Many2one('ir.attachment', domain="[('res_model', '=', 'thmdocument.task'), ('res_id', '=', id), ('mimetype', 'ilike', 'image')]", string='Displayed Image')
    ...
    def _onchange_thmdocument(self):
        if self.thmdocument_id:
            self.partner_id = self.thmdocument_id.partner_id
            self.stage_id = self.stage_find(self.thmdocument_id.id, [('fold', '=', False)])
        else:
            self.stage_id = False
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        obj_tm = self.env.user.company_id.thmdocument_time_mode_id
        ...
        # if the thmdocument_time_mode_id is not in hours (so in days), display it as a float field
    ...
    def get_empty_list_help(self, help):
        self = self.with_context(
            empty_list_help_id=self.env.context.get('default_thmdocument_id'),
            empty_list_help_model='thmdocument.thmdocument',
    ...
    def stage_find(self, section_id, domain=[], order='sequence'):
        ...
        section_ids.extend(self.mapped('thmdocument_id').ids)
        search_domain = []
        if section_ids:
            search_domain = [('|')] * (len(section_ids) - 1)
            for section_id in section_ids:
                search_domain.append(('thmdocument_ids', '=', section_id))
        search_domain += list(domain)
        # perform search, return the first found
        return self.env['thmdocument.task.type'].search(search_domain, order=order, limit=1).id
    ...
    def create(self, vals):
        ...
        # for default stage
        if vals.get('thmdocument_id') and not context.get('default_thmdocument_id'):
            context['default_thmdocument_id'] = vals.get('thmdocument_id')
    ...
    def _track_subtype(self, init_values):
        ...
        if 'kanban_state' in init_values and self.kanban_state == 'blocked':
            return 'thmdocument.mt_task_blocked'
        elif 'kanban_state' in init_values and self.kanban_state == 'done':
            return 'thmdocument.mt_task_ready'
        elif 'user_id' in init_values and self.user_id:  # assigned -> new
            return 'thmdocument.mt_task_new'
        elif 'stage_id' in init_values and self.stage_id and self.stage_id.sequence <= 1:  # start stage -> new
            return 'thmdocument.mt_task_new'
        elif 'stage_id' in init_values:
            return 'thmdocument.mt_task_stage'
        return super(Task, self)._track_subtype(init_values)

    @api.multi
    def _notification_recipients(self, message, groups):
        """ Handle thmdocument users and managers recipients that can convert assign
    ...
    def message_get_reply_to(self, res_ids, default=None):
        """ Override to get the reply_to of the parent thmdocument. """
        tasks = self.sudo().browse(res_ids)
        thmdocument_ids = tasks.mapped('thmdocument_id').ids
        aliases = self.env['thmdocument.thmdocument'].message_get_reply_to(thmdocument_ids, default=default)
        return {task.id: aliases.get(task.thmdocument_id.id, False) for task in tasks}

    @api.multi
    def email_split(self, msg):
        email_list = tools.email_split((msg.get('to') or '') + ',' + (msg.get('cc') or ''))
        # check left-part is not already an alias
        aliases = self.mapped('thmdocument_id.alias_name')
    ...
    def message_get_email_values(self, notif_mail=None):
        ...
        if self.thmdocument_id:
            ...
            current_objects.insert(0, 'thmdocument.thmdocument-%s, ' % self.thmdocument_id.id)
...
class AccountAnalyticAccount(models.Model):
    ...
    def _compute_thmdocument_count(self):
        for account in self:
            account.thmdocument_count = len(account.with_context(active_test=False).thmdocument_ids)

    @api.model
    def _trigger_thmdocument_creation(self, vals):
        '''
    ...
    def thmdocument_create(self, vals):
        '''
        This function is called at the time of analytic account creation and is used to create a thmdocument automatically linked to it if the conditions are meet.
        '''
        self.ensure_one()
        Thmdocument = self.env['thmdocument.thmdocument']
        thmdocument = Thmdocument.with_context(active_test=False).search([('analytic_account_id', '=', self.id)])
        if not thmdocument and self._trigger_thmdocument_creation(vals):
            thmdocument_values = {
                'name': vals.get('name'),
    ...
    def create(self, vals):
        ...
        analytic_account.thmdocument_create(vals)
        return analytic_account

    @api.multi
    def write(self, vals):
        vals_for_thmdocument = vals.copy()
        for account in self:
            if not vals.get('name'):
                vals_for_thmdocument['name'] = account.name
            account.thmdocument_create(vals_for_thmdocument)
        return super(AccountAnalyticAccount, self).write(vals)

    @api.multi
    def unlink(self):
        thmdocuments = self.env['thmdocument.thmdocument'].search([('analytic_account_id', 'in', self.ids)])
        has_tasks = self.env['thmdocument.task'].search_count([('thmdocument_id', 'in', thmdocuments.ids)])
        if has_tasks:
            raise UserError(_('Please remove existing tasks in the thmdocument linked to the accounts you want to delete.'))
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        if args is None:
            args = []
        if self.env.context.get('current_model') == 'thmdocument.thmdocument':
    ...
    @api.multi
    def thmdocuments_action(self):
        thmdocuments = self.with_context(active_test=False).mapped('thmdocument_ids')
        result = {
            "type": "ir.actions.act_window",
            "res_model": "thmdocument.thmdocument",
            "views": [[False, "tree"], [False, "form"]],
            "domain": [["id", "in", thmdocuments.ids]],
            "context": {"create": False},
            "name": "Thmdocuments",
        }
        if len(thmdocuments) == 1:
            result['views'] = [(False, "form")]
            result['res_id'] = thmdocuments.id
        else:
